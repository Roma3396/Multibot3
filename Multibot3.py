...
async def delete_process(message: types.Message, state: FSMContext):
    ...
    if film:
        ...
        c.execute("DELETE FROM favorites WHERE film_id = ?", (film[0],))
        conn.commit()
        await message.answer(f"✅ Film (Kod: {code}) o'chirildi!", reply_markup=main_menu(message.from_user.id))
        await state.clear()
    else:
...
async def film_actions(call: types.CallbackQuery):
    ...
    if action == "like":
        ...
        if film:
            text = f"🎬 **{film[3]}**\n\n📅: {film[4]}\n🔢: {film[5]}\n📝: {film[6]}\n\n❤️ {film[7]} ta"
            kb = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="⬅️", callback_data=f"prev_{film[0]}"),
                 InlineKeyboardButton(text=f"❤️ {film[7]}", callback_data=f"like_{film[0]}"),
                 InlineKeyboardButton(text="💾", callback_data=f"save_{film[0]}"),
                 InlineKeyboardButton(text="➡️", callback_data=f"next_{film[0]}")],
                [InlineKeyboardButton(text="👁 Tomosha", callback_data=f"watch_{film[0]}")]
            ])
            await call.message.edit_media(types.InputMediaPhoto(media=film[1], caption=text, parse_mode="Markdown"), reply_markup=kb)
        else: await call.answer("Boshqa yo'q!", show_alert=True)
    conn.close()

@dp.message(F.text == "🔍 Qidiruv")
async def search_start(message: types.Message, state: FSMContext):
    await state.set_state(UserState.waiting_for_search)
    await message.answer("Nom yoki kod:", reply_markup=back_kb())
...
async def show_rek(message: types.Message):
    conn = sqlite3.connect('films.db')
    c = conn.cursor()
    c.execute("SELECT * FROM films ORDER BY RANDOM() LIMIT 1")
    film = c.fetchone()
    conn.close()
    if film: await send_film_card(message.chat.id, film)
...
async def main():
    # 1. Veb-serverni orqada ishga tushiramiz (Render uchun)
    asyncio.create_task(start_webhook_server())
    
    # 2. Botni ishga tushiramiz
    logging.info("Bot polingni boshladi...")
